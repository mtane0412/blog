...
const amazonList = yaml.safeLoad(fs.readFileSync('content/amazon-list.yaml'), 'utf8');

const html = ({title, imageUrl, price, url,updatedAt}) => {
  const updatedAtYYYYMMDD = moment(updatedAt).format("YYYY年MM月DD日");
  if (title && imageUrl && price) {
  return `<div style="display: flex; background-color: #fff; padding: 1em; margin: 1em; border: 1px solid #ccc;align-items: center; flex-wrap: wrap;">
...
const updateAmazonListYaml = async (amazonList) => {
  fs.writeFile('content/amazon-list.yaml', yaml.dump(amazonList), 'utf8', (err) => {
    if (err) {
